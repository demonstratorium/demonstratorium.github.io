...
const _ = { DATA     : {}
     ...
     // ----------------------------------------------Hierarchie RDF Part-------
                   ...
                   // ,tnd = document.createTextNode(' [ d ]') //Delete TextNode wenn parents vorhanden sindDATA[tx]?.['@HC']
                    ...
                    if ((DATA?.[tx]?.['@HC']||[]).length)  // NUR wenn TextNode vorhanden ist
                      { spa.setAttribute('data-counter',(DATA?.[tx]?.['@HC']||[]).length);//2024-07-19 css bullet
                        const spd = _.Y.setIcon('vw-reload','icon','nur löschen, wenn aufgeklappt')//document.createElement('span')
         ...
         ln.appendChild( _.Y.setIcon('vw-link','icon','Dokument mit Rechtsklick in neuem Tab öffnen'));//document.createTextNode('Link'));//SpäterIcon
       ...
       return f;} 
         ...
         ,i2c: (e)=>{ // Modal ohne inner HTMl
          ...
          dv.appendChild( _.M.gh( 'i2c Modal','Form to Table'               ) ); // get Header
          ...
          _.fbtnsave= async (event) =>{ //HIER könnte man die Möglichkeiten sihe bt bt 1 nutzen!!!ÜBERDENKENNNNNNN 2024-07-05
            try{// der TRY ist wichtig, damit Modal beendet werden kann
             ...
             // fj['PID'] = 'PROJECTE';
              ...
              // console.time('MX');
              //console.log('FBTSAVE j',fj);
              const ct=performance.now()
              , r = await _.C.m2eda ( _.U.j2Dj( fj ))
              ;
              //console.timeEnd('MX');
              //console.log('RESPONSE ',r);//ev fehler
              document.querySelector('#info').textContent=`for ID :${r.Entries[0].EventId} in ${parseInt(performance.now()-ct)} ms `;
...
/*
  ...
  ,i2c : (v,c,i,a,m='vw-visualcheck',k,o)=>{ const sv = _.Y.setIcon(m,'s-icon','mit SID= leer wird ein neuer Datensatz angelegt, sonst die Felder geupdated'); // i2c icon to create Row row to detail k PID|SID|D|WAS PID wird idd
                                              sv.addEventListener('click',()=>{ _.M.i2c({k:k,o:o});
                                                                                //console.log(`Y.i2m v${v} a ${a} c${c} klick modal`)
                                                                              });
                                              return sv;         
                                          }   
...
return sv;  
   ...
   ,gF : (v)=>{ const sv = _.Y.setIcon('vw-filter'); //aktuell NIchct sehr elegant, funzt aber
     ...
     , df : (v,c,i,a,k,o,m )=>{ const e = document.createElement ('span') //doubleFilter //Durch-Reichen des MInMax
                               ,   vmm = parseInt(10*(v-m?.min||0)/(m?.max||1-m?.min||0))
                               ;//console.log(m,vmm);
                                   e.setAttribute('class',`u-filter bszn${vmm||1}`);
                                   e.setAttribute('title',`SUM\n${v||0}`);
                          e.addEventListener('click',()=>{ //console.log(`v ${v} a ${a} c ${c} k${k} k${o} klick modal`); //v 31 a slei c GIA kprodukt|schicht|GIA|GTB|EBA|GIB|GTA|EBB k337 600 010 TG||31|56|15|45|43|13 klick modal
                                                           _.T.s2F(a
...
//                          e.appendChild(document.createTextNode(v?Number(v/Math.pow(10,Math.floor(Math.log10(v)))).toFixed(1)+'E'+Math.floor(Math.log10(v)) :'')) // ##2023-08-30 Grosse Zahlen im GrpSum besser darstellen
     ...
     ,svgBarCode : i => { const  _vbc = ( valuetobarcode ) => {
          ...
          , L : { version : () =>`ìnfo T 0.1 layout` //_.T.next('mat')
                ...
                , "X"    : (v,c,i,a ,k,o) => _.Y.del(v,c,i,a,'vw-delete',k,o)//(v,i,t,n,I,A,ut)=>`<a href="javascript:_.U.del('${A._id}','${ut}','${A.TP||'XTP'}')" title="Datensatz löschen"><svg class=s-icon><use xlink:href="#vw-delete" /></svg></a>`
                , "U"    : (v,c,i,a ,k,o) => _.Y.upd(v,c,i,a,'vw-pencil',k,o)//(v,i,t,n,I,A,ut)=>`<a href="javascript:_.U.del('${A._id}','${ut}','${A.TP||'XTP'}')" title="Datensatz löschen"><svg class=s-icon><use xlink:href="#vw-delete" /></svg></a>`
                , "up1"  : (v,c,i,a ,k,o) => _.Y.up(v,c,i,a,'vw-arrow-forward',k,o,1)//(v,i,t,n,I,A,ut)=>`<a href="javascript:_.U.del('${A._id}','${ut}','${A.TP||'XTP'}')" title="Datensatz löschen"><svg class=s-icon><use xlink:href="#vw-delete" /></svg></a>`
                , "um1"  : (v,c,i,a ,k,o) => _.Y.up(v,c,i,a,'vw-arrow-back',k,o,-1)//(v,i,t,n,I,A,ut)=>`<a href="javascript:_.U.del('${A._id}','${ut}','${A.TP||'XTP'}')" title="Datensatz löschen"><svg class=s-icon><use xlink:href="#vw-delete" /></svg></a>`
/*Info Update Delete*/,"iud" : (v,c,i,a,k,o)=> _.Y.iud(v,c,i,a,'vw-delete',k,o)
,"vpm" : (v,c,i,a,k,o)=> _.Y.vpm(v,c,i,a,'vw-delete',k,o)//Value plus 1 minus 1 für workflows in
...
//              ,stats : e =>{const J=X['boxplot'](d['fs_'+e.t].map(E=>{E['I'+e.col]=(typeof E[e.col] ==='string')?(parseFloat(E[e.col].replace(/,/,"."))||0):E[e.col];return E;}),{structure:['I'+e.col]})['I'+e.col];return `<span class=info>&Sigma; ${J.s.toFixed(1)}<br/>[${J.min},${J.max}]</span>`;}//X['boxplot'](d.fs_mabestat,{"structure":["ME"]})
  ...
  , C:{ version : ()=> `version 0.2 C Anbindung Cloud 2024-07-06..`
      , APIID   : 'XX' 
      , REGION  : 'XX' 
      , PUT     : 'XX'
      , UPD     : 'XX'
